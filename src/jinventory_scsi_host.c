...
int jinventory_num_scsi_hosts( void )
{
	...
	num_scsi_hosts = jinventory_scsi_host_info(0,&json_str,(void **)NULL); 
	if (json_str)
		free(json_str);
...
int jinventory_scsi_hosts_show_json( void )
{
	char *json_str = NULL;
	int num_scsi_hosts = 0;
	num_scsi_hosts = jinventory_scsi_host_info(PFSD_INV_FLAG_VERBOSE, (char **)NULL, (void **)NULL); 
	/*	num_scsi_hosts = jinventory_scsi_host_info(PFSD_INV_FLAG_VERBOSE, &json_str, (void **)NULL);  */
...
{
	char *json_str = NULL;
	jinventory_scsi_host_info(0, &json_str, (void **)NULL);
	return json_str;
}
...
void *jinventory_scsi_hosts_get_json_object( void )
{
	char *json_str = NULL;
	json_object *jobj;
	jinventory_scsi_host_info(0, &json_str, (void **)&jobj);
	return (void *)jobj;
}
 ...
 * **********************************************************************
 *
 * Function:  int jinventory_scsi_host_info()
 *
 * Description:
 *
 * inputs:
