...
int jinventory_num_drives( void )
{
	...
	/* Don't ask me why but doing it like this causes a segfault of the variety...
	   ...
	   num_drives = jinventory_drive_info(0,(char **)NULL,(void **)NULL); 
	   The CPU one does the exact same thing.
	*/

	num_drives = jinventory_drive_info(0,&json_str,(void **)NULL); 
...
int jinventory_drives_show_json( void )
{
	char *json_str = NULL;
	int num_drives = 0;
	num_drives = jinventory_drive_info(PFSD_INV_FLAG_VERBOSE, (char **)NULL,(void **)NULL); 
	/* num_drives = jinventory_drive_info(PFSD_INV_FLAG_VERBOSE, &json_str,(void **)NULL);  */
	if (json_str)
		free(json_str);
	return num_drives;
}
...
{
	char *json_str = NULL;
	jinventory_drive_info(0, &json_str,(void **)NULL); 
	return json_str;
}
...
int jinventory_num_cpus( void )
{
	char *json_str = NULL;
	int num_cpus = 0;
	num_cpus = jinventory_cpu_info(0,&json_str,(void **)NULL); 
	if (json_str)
		free(json_str);
	return num_cpus;
}
...
int jinventory_cpus_show_json( void )
{
	char *json_str = NULL;
	int num_cpus = 0;
	num_cpus = jinventory_cpu_info(PFSD_INV_FLAG_VERBOSE, (char **)NULL, (void **)NULL); 
	/* num_cpus = jinventory_cpu_info(PFSD_INV_FLAG_VERBOSE, &json_str, (void **)NULL);  */
	if (json_str)
		free(json_str);
	return num_cpus;
}
...
int jinventory_num_nets( void )
{
	char *json_str = NULL;
	int num_nets = 0;
	num_nets = jinventory_net_info(0,&json_str,(void **)NULL); 
	if (json_str)
		free(json_str);
	return num_nets;
}
...
int jinventory_nets_show_json( void )
{
	char *json_str = NULL;
	int num_nets = 0;
	num_nets = jinventory_net_info(PFSD_INV_FLAG_VERBOSE, (char **)NULL, (void **)NULL); 
	/*	num_nets = jinventory_net_info(PFSD_INV_FLAG_VERBOSE, &json_str, (void **)NULL);  */
...
{
	char *json_str = NULL;
	jinventory_net_info(0, &json_str, (void **)NULL);
	return json_str;
}
...
{
	char *json_str = NULL;
	jinventory_netstat_info(0, &json_str, iface, (void **)NULL);
	return json_str;
}
...
{
	char *json_str = NULL;
	jinventory_net_interfaces(0, &json_str, (void **)NULL);
	return json_str;
}
...
char *jinventory_inventory_get_json_str( void )
{
	...
	strcpy(dstr,"storage");
	jobj = jinventory_drives_get_json_object();
	json_object_object_add(inventory_object, dstr, jobj);

	strcpy(dstr,"cpu");
	jobj = jinventory_cpus_get_json_object();
	json_object_object_add(inventory_object, dstr, jobj);

	strcpy(dstr,"network");
	jobj = jinventory_nets_get_json_object();
	json_object_object_add(inventory_object, dstr, jobj);

#if defined ENABLE_SCSI_HOST
	strcpy(dstr,"scsi_host");
	jobj = (json_object *)jinventory_scsi_hosts_get_json_object();
	json_object_object_add(inventory_object, dstr, jobj);
#endif
 ...
 * **********************************************************************
 *
 * Function: int jinventory_drive_info()
 *
 * Description:
 *
 * inputs: 
 ...
 * **********************************************************************
 *
 * Function: char *jinventory_disk_property_get
 *
 * Description:
 *
 * inputs: 
...
char *jinventory_disk_property_get( unsigned int flags, char *inproperty, char *indevname )
{
	...
	udev_list_entry_foreach(dev_list_entry, devices) {
		...
		if (id_bus != NULL){
			if(strcmp(id_bus, "disk") == 0){
				property =  udev_device_get_property_value(dev, "DEVNAME");
				if (strcmp(property, indevname) == 0 ){
 ...
 * **********************************************************************
 *
 * Function: char *jinventory_disk_sysattr_get
 *
 * Description:
 *
 * inputs: 
 ...
 * **********************************************************************
 *
 * Function:  int jinventory_cpu_info()
 *
 * Description:
 *
 * inputs:
 ...
 * **********************************************************************
 *
 * Function:  int jinventory_net_info()
 *
 * Description:
 *
 * inputs:
	...
	udev_list_entry_foreach(dev_list_entry, devices) {
		...
		*/
		if ((sock >= 0) && (strstr(path, "virtual") == NULL)) {
			if ( jinventory_get_eth_drvinfo( sock, netname, &drvinfo) > 0){
 ...
 * **********************************************************************
 *
 * Function:  int jinventory_netstat_info()
 *
 * Description:
 *
 * inputs:
 ...
 * **********************************************************************
 *
 * Function:  int jinventory_net_interfaces()
 *
 * Description:
 *
 * inputs:
...
static json_object *jinventory_drives_get_json_object( void )
{
	char *json_str = NULL;
	json_object *jobj;
	jinventory_drive_info(0, &json_str, (void **)&jobj);
	return jobj;
}
...
static json_object *jinventory_cpus_get_json_object( void )
{
	char *json_str = NULL;
	json_object *jobj;
	jinventory_cpu_info(0, &json_str, (void **)&jobj);
	return jobj;
}
